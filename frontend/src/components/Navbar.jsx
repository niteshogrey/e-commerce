...
const Navbar = () => {
  ...
  return (
    <div className='h-20 shadow-xl flex items-center px-12 justify-evenly'>
        <div>
        ...
        <nav className='flex gap-6 text-xl font-semibold'>
            <Link key="home" className={`${active === "home" && "underline text-blue-500"}  `} onClick={()=>setActive("home")} >Home</Link>
            <Link key="product" className={`${active === "product" && "underline text-blue-500"} `} onClick={()=>setActive("product")} >Product</Link>
        ...
        <button onClick={()=>dispatch(logout())} className='flex text-lg items-center gap-2 bg-red-700 text-white font-semibold px-2 py-1 rounded-full cursor-pointer'>
            Sign Out
            <LogOut />
        </button>
    </div>
